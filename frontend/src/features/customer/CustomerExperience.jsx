...
export function CustomerExperience() {
  ...
  const removeFromOrder = (indexToRemove) => {
    updateSession((session) => ({
      ...session,
      orderItems: session.orderItems.filter((_, index) => index !== indexToRemove)
    }));
  };
  ...
  return (
    <section className="customer-layout customer-layout--immersive">
      <article className="card customer-column customer-column--voice">
        ...
        <p className="caption">Hands-free ordering from your table.</p>
        <label htmlFor="restaurant-picker">Restaurant Session</label>
        <select
        ...
        <div className="voice-orb voice-orb--active" role="status" aria-live="polite">
          Listening now…
        </div>
      ...
      <article className="card customer-column customer-column--menu">
        ...
        <div className="recommendation-list recommendation-list--tablet-fit">
          {recommendations.map((entry) => (
            <section key={entry.title} className="recommendation recommendation--tablet">
              <img className="recommendation__image" src={entry.image} alt={entry.title} />
      ...
      <article className="card customer-column customer-column--order">
        <h2>Order List</h2>
        {orderItems.length === 0 ? (
          <p className="caption">No items yet. Add dishes from recommendations.</p>
        ) : (
                ...
                <div>
                  ...
                  <div className="tag-row">
                    {item.dietaryTags.map((tag) => (
                      <span key={`${item.title}-${tag}-${index}`} className="dietary-tag">
                        {tag}
                      </span>
                    ))}
                  </div>
      ...
      {selectedMenu && (
        <div className="modal-overlay" role="dialog" aria-modal="true" aria-label="Menu detail popup">
      ...
      {isCheckoutOpen && (
        <div className="modal-overlay" role="dialog" aria-modal="true" aria-label="Finalize order confirmation">
          <section className="modal-card">
            <h3>Finalize your order?</h3>
            <p>You can confirm now, or continue thinking about the menu.</p>
            <div className="modal-actions">
              <button type="button" className="primary-button primary-button--active" onClick={finalizeOrder}>
                Confirm order
              </button>
              <button type="button" className="primary-button" onClick={() => setIsCheckoutOpen(false)}>
                Keep exploring menu
              </button>
            </div>
          </section>
